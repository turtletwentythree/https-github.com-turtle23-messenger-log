...
docs/booking_data.json         ← {generated_at, count, records:[...]} — หน้าเว็บ fetch ไฟล์นี้เองเพื่อซิงก์สด
```
...
- Workflow รันอัตโนมัติ**ทุก 5 นาที** (`*/5 * * * *`, เวลา UTC — ความถี่สูงสุดที่ GitHub
  Actions รองรับสำหรับ scheduled workflow) — แก้ความถี่ได้ที่ `.github/workflows/sync.yml`
  บรรทัด `cron:` (GitHub อาจดีเลย์การรันไม่กี่นาทีช่วงโหลดสูง เป็นเรื่องปกติ)
- กด **Actions → Sync Messenger booking data → Run workflow** เพื่อสั่งซิงก์ทันทีได้ตลอด
- **หน้าเว็บเองก็ซิงก์ข้อมูลสดอีกชั้นหนึ่ง** โดยไม่ต้องรอ workflow: ทุกครั้งที่เปิดหน้า
  (หรือสลับกลับมาที่แท็บ หรือเปิดค้างไว้นานเกิน 5 นาที) หน้าเว็บจะ fetch
  `docs/booking_data.json` ใหม่เองอัตโนมัติแล้วอัปเดตตาราง/สถิติให้ทันทีโดยไม่ต้อง
  reload หน้า — และมีปุ่ม **"🔄 ซิงค์ข้อมูลล่าสุด"** มุมขวาบนให้กดซิงก์เองได้ทุกเมื่อ
  (ทั้งหมดนี้เป็นแค่การดึงไฟล์ JSON ที่ build ไว้แล้วมาแสดง ไม่ได้ต่อ SharePoint ตรงจาก
  เบราว์เซอร์ — ปลอดภัย ไม่ต้องมี credential ใดๆ ฝังอยู่ในหน้าเว็บสาธารณะ)
